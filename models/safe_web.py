...
local_hr = HR(_style='margin-top: 10px; margin-bottom: 0px; '
                     'border: 0; border-top: 1px solid #325d88;')
...
vacancy_type = ['Technical Intern', 'Field Intern', 'Volunteer', 'Field Assistant',
                'PhD Position', 'Other']
...
transfer_set = ['Tawau to SAFE', 'SAFE to Tawau', 'SAFE to Maliau',
                'Maliau to SAFE', 'Maliau to Tawau', 'Tawau to Maliau',
                'SAFE to Danum Valley', 'Danum Valley to SAFE']

# Make transfer set available to modules.
# This is a bit of a hack. It would be cleaner to have a module full of these global variables
# and then import them as required, but that requires a lot of rewriting!
current.transfer_set = transfer_set
...
db.define_table('project_details',
                ...
                Field('thumbnail_figure', 'upload',
                      uploadfolder=os.path.join(request.folder, 'uploads/images/projects')),
                ...
                Field('admin_status', 'string', requires=IS_IN_SET(project_status_set),
                      default='Pending'),
...
db.define_table('outputs',
                Field('thumbnail_figure', 'upload',
                      uploadfolder=request.folder + '/uploads/images/outputs'),
                ...
                # The fields below are to handle approval of new records
                Field('admin_status', 'string', requires=IS_IN_SET(admin_status_set),
                      default='Pending'),
...
db.gazetteer_alias._before_insert.append(lambda r : 
                                            db((db.gazetteer_alias.zenodo_record_id == r["zenodo_record_id"]) &
...
db.define_table('research_assistant_bookings',
                ...
                Field('work_type', 'string',
                      requires=IS_IN_SET(['Standard', 'Rope work', 'Night work'])))
...
db.define_table('species_profile',
                ...
                Field('local_abundance', 'string',
                      requires=IS_IN_SET(
                          ['commonly seen', 'often seen', 'sometimes seen', 'rarely seen'])),
                ...
                Field('image_link', 'string', requires=IS_NULL_OR(IS_URL())),
                # e.g. link to static flickr farm
                Field('image_href', 'string', requires=IS_NULL_OR(IS_URL())),
                # e.g. link to flickr user photo page
...
db.define_table('blog_posts',
                Field('thumbnail_figure', 'upload',
                      uploadfolder=request.folder + '/uploads/images/blog_thumbnails'),
...
db.define_table('news_posts',
                Field('thumbnail_figure', 'upload',
                      uploadfolder= request.folder+'/uploads/images/news_thumbnails'),
                Field('title', 'string'),
                Field('content', 'text', widget=ckeditor.widget),
                # who posted it and when
